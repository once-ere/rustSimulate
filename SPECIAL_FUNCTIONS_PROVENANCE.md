...
- **Uniform asymptotics for large |z| at non-integer order.** The
  ...
  along the real axis.
- **Complex order in the large-order expansions.** The Debye,
  Airy-type and `1/z` machinery are expansions *in* the order, and their
  uniformity is stated for real order. Complex order reaches as far as
  the ascending series does and no further.
