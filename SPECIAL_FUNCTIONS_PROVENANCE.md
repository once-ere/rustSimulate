...
| `bessel_complex` (integer Y) | **found in Stage 18, not fixed** | `bessel_y_c` builds `Y_n` by upward recurrence in `n`, which is stable for real argument but not near the **imaginary** axis, where `Y_n` is a combination whose recessive part the recurrence amplifies. At `n = 2, z = 29.4e^{1.6i}` the `1/z` expansion closes the J–Y Wronskian to 7e-26 and the integer series to 4.5e-6. The Stage-13 accuracy law does not describe this. `bessel_cnu` now carries an `exp(\|Im z\|)` term so the selector stops trusting that route there; **fixing `bessel_y_c` itself is outstanding** |
...
- **Uniform asymptotics for large |z| at non-integer order.** The
  ...
  along the real axis.
- **Complex Airy**, and with it DLMF 10.20 at complex order. This is now
  the only thing standing between the crate and the turning point
  `|z| ~ |ν|` at complex order — the expansion is understood, its
  ingredient is not implemented.
- **A fix for `bessel_y_c` near the imaginary axis** (see the accuracy
  table). Guarded against, not repaired.
