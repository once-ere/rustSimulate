...
- **The negative real axis at large `|z|`.** Both `1/z` Hankel
  expansions keep a `π/3` margin from it, so past `|z| ≈ 25` there `Y`
  and `K` fall back to the ascending series and its `exp(|z|)` loss.
  It is the branch cut, where the value is a convention anyway, but it
  is the one direction Stage 19 did not reach.
