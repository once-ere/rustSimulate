...
- **Uniform asymptotics for large |z| at non-integer order.** The
  ...
  along the real axis.
- **The Airy-type uniform expansion, DLMF 10.20.** Deliberately not
  added: measured across `z/ν` from 0.95 to 1.1 the existing routes
  give 1e-14, so it has nothing to fix, and truncated at `A₀, B₀` it
  carries `O(ν⁻²)` ≈ 6e-6 at `ν = 400` — it would widen coverage while
  lowering the floor. The one thing it would reach is where the value
  is outside `f64` anyway.
