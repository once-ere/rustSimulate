...
$K_0$ is worthless past about $x=12$. The rule of thumb that is actually
safe is $|z|\le 10$ for every kind. The $J$ and $I$ rows agree digit for
digit because they are the same measurement --- $I_n(z)$ \emph{is}
$J_n(iz)$.
...
The uniform \textbf{Airy-type} expansion of DLMF 10.20 --- uniform
\emph{through} the turning point $z \sim \nu$ --- is \textbf{not}
implemented, and the reason is measured rather than assumed: across
$z/\nu$ from 0.95 to 1.1 the existing routes already give $10^{-14}$, so
there is nothing there to fix. Truncated at its first two coefficients
Olver's expansion carries a relative error of $O(\nu^{-2})$ --- about
$6\times10^{-6}$ at $\nu = 400$ --- so adding it would widen coverage
while lowering the floor. Complex \emph{order} is also still absent.
...
\subsection{\kw{scene} --- the graphical scene window}
...
\code{QM STEP <dt>} / \code{QM RUN <t> [STEPS <n>]} & Crank--Nicolson propagation\\
\code{QM NORM ENERGY POSITION MOMENTUM} & observables\\
...
\textbf{The walls are infinite and they reflect.} \code{QM GRID} pins
...
\code{QM2 NORM | ENERGY | CENTROID} & observables\\
...
\code{QM3 NORM | ENERGY | CENTROID} & observables\\
