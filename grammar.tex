...
both estimates and what is missing. That is the substantive difference
from the plain forms, which returned a confident wrong first digit. The
gap is the region where $|z|$ and $\nu$ are large and comparable, which
needs the uniform Airy-type expansions of DLMF 10.20 --- \textbf{not
implemented}. Complex \emph{order} is also still absent.
