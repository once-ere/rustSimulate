...
Two functions are \textbf{not} exposed, and the reason is worth stating
rather than leaving you to discover it: \code{solve\_tridiag\_c} and
\code{solve\_cyclic\_tridiag\_c} are complex-valued, and this language
has no complex type. Reaching them needs a new value kind plus literal
syntax in the lexer, which is a language change rather than a
registration; it is staged deliberately instead of being half-done. The
real-valued \code{solve\_tridiag} is available now.
...
\section{Fifteen worked examples}
