...
| | |
|---|---|
| Upstream | `once-ere/sundials_rs` @ `faabb7f` |
| What it is | A pure-Rust, zero-`unsafe`, dependency-free translation of SUNDIALS 7.7.0 (LLNL) |
| Licence | BSD-3-Clause, as declared in its workspace `Cargo.toml` |
| Modified here? | **No.** 394 files, byte-identical (git subtree hash `f616de8…`) |

Provenance and the byte-identity proof are in
